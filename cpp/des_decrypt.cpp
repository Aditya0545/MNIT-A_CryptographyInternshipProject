...
string circular_left_shift_2(string key_chunk)
{
	string shifted = "";
	for (int i = 0; i < 2; i++)
	{
...
void generate_keys(string key)
{
	...
	for (int i = 0; i < NUM_OF_ROUNDS; i++)
	{
		...
		if (i == 0 || i == 1 || i == 8 || i == 15)
		{
			left = circular_left_shift_1(left);
			right = circular_left_shift_1(right);
		}
		// 3.2. For other rounds, the key_chunks
		// are shifted by two
		else
		{
			left = circular_left_shift_2(left);
			right = circular_left_shift_2(right);
		}
