...
def permute(block, table):
    """Permute the block using the given permutation table"""
...
def s_box_substitution(bits):
    ...
    for i in range(8):  # 8 S-boxes
        ...
        output.extend([int(b) for b in format(s_value, "04b")])  # 4-bit output
    return output
...
def generate_keys(main_key):
    # Key schedule implementation (placeholder, requires parity bit handling)
    return [main_key[:48] for _ in range(16)]  # Example: repeat same key 16 times

def des_encrypt(block, keys):
    block = permute(block, IP)
    left, right = block[:32], block[32:]
    for round_key in keys:
        new_right = xor(left, feistel_function(right, round_key))
        left, right = right, new_right  # Swap halves
    combined = right + left  # Note: No swap after final round
    return permute(combined, FP)

def des_decrypt(block, keys):
    return des_encrypt(block, keys[::-1])  # Same as encryption but reverse keys
...
if __name__ == "__main__":
    plaintext = "ABCDEFGH"  # Example 64-bit input
    key = "simplekey"  # Example key
    plaintext_bits = string_to_bit_array(plaintext)
    key_bits = string_to_bit_array(key)

    # Pad or truncate key to 64 bits (example)
    ...
    # Encrypt
    encrypted_bits = des_encrypt(plaintext_bits, keys)
    encrypted_text = bit_array_to_string(encrypted_bits)
    print(f"Encrypted: {binascii.hexlify(bytearray(encrypted_text, 'utf-8')).decode()}")
